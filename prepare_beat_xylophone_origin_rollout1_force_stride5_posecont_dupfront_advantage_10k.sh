...
ROLLOUT_DATA_DIR="/mnt/public/gaojiahuan/liushengbang/Data/Rollout/beat_xylophone1"
ROLLOUT_CSV="$ROLLOUT_DATA_DIR/annotation.csv"
ROLLOUT_SUCCESS_DIR="/mnt/public/gaojiahuan/liushengbang/Data/Rollout/beat_xylophone1_success_only"
OUTPUT_ROOT="/mnt/public/gaojiahuan/liushengbang/Data/processed_lerobot/wan_worldrl_stride5_posecont_192x256"
REPO_NAME="beat_xylophone_origin_rollout1_force_stride5_posecont_dupfront_192x256_success_advantage"
CONFIG_NAME="pi05_beat_xylophone_origin_rollout1_force_stride5_posecont_dupfront_192x256_advantage_10k"
...
print(f"rollout1: success={count}, marked_fail={len(marked)}")
PY
success_count="$(find "$ROLLOUT_SUCCESS_DIR" -maxdepth 1 -type l -name '*.hdf5' | wc -l)"
if [[ "$success_count" != "27" ]]; then
  echo "Expected 27 successful rollout symlinks, got $success_count" >&2
  exit 1
fi

echo "[prepare] Rebuilding origin + beat_xylophone1 successful rollout LeRobot dataset -> $OUTPUT_ROOT/$REPO_NAME"
